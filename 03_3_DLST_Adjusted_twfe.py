...
def plot_metric_transition_side_by_side(
    coef_all: pd.DataFrame,
    metric_name: str,
    metric_label: str,
    units: str,
    ...
    """Side-by-side coefficient panels for spring vs fall (all_days only)."""
    fig, axes = plt.subplots(1, 2, figsize=(13.5, 5.2), sharex=True, sharey=True)
    transitions = ["spring", "fall"]

    for i, transition in enumerate(transitions):
        ax = axes[i]
        sdf = coef_all[
            (coef_all["transition"] == transition)
            & (coef_all["metric"] == metric_name)
            & (coef_all["day_type"] == "all_days")
        ].sort_values("event_day")

        if sdf.empty:
            ax.text(0.5, 0.5, "No estimable coefficients", ha="center", va="center", transform=ax.transAxes)
            ax.set_title(f"{transition.title()} (no data)")
            ax.axvline(0, color="black", linestyle="--", linewidth=1)
            ax.axhline(0, color="gray", linestyle=":", linewidth=1)
            continue
        ...
        ax.axvline(0, color="black", linestyle="--", linewidth=1.2)
        ax.axhline(0, color="gray", linestyle=":", linewidth=1)
        ax.set_title(f"{transition.title()} transition")
        ax.set_xlabel("Event day")
        ax.grid(alpha=0.25)

    axes[0].set_ylabel(f"DiD coefficient ({units})")
    fig.suptitle(
        f"TWFE event-study coefficients: {metric_label}\nSpring vs Fall (all days)",
        fontsize=12,
        y=1.02,
    )
    plt.tight_layout()
    out_path = plot_dir / f"eventstudy_compare_{metric_name}_spring_vs_fall.png"
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def run_analysis(
        ...
        for metric_name, meta in metric_meta.items():
            ...
            for daytype_name, daytype_filter in DAYTYPE_SPECS:
                if method == "low_memory":
                    ...
                    tdf_day = tdf.copy()
                    if daytype_filter is True:
                        tdf_day = tdf_day[tdf_day["is_work_day"] == 1].copy()
                    elif daytype_filter is False:
                        tdf_day = tdf_day[tdf_day["is_work_day"] == 0].copy()

                    if not tdf_day.empty:
                        s = (
                            tdf_day.groupby(["event_day", "dst_state"], as_index=False)
                            .agg(
                                n_rows=("person_id", "size"),
                                n_people=("person_id", "nunique"),
                                n_dates=("sleep_date", "nunique"),
                            )
                            .sort_values(["event_day", "dst_state"])
                        )
                        s["transition"] = transition
                        s["metric"] = metric_name
                        s["metric_label"] = meta["label"]
                        s["day_type"] = daytype_name
                        support_parts.append(s[["transition", "metric", "metric_label", "day_type", "event_day", "dst_state", "n_rows", "n_people", "n_dates"]])
    ...
    if support_parts:
        support_all = pd.concat(support_parts, ignore_index=True)
        support_all = support_all.sort_values(["transition", "metric", "day_type", "event_day", "dst_state"]).reset_index(drop=True)
        support_all.to_csv(table_dir / "05_eventday_support.csv", index=False)
    ...
    if not coef_all.empty:
        ...
        flags = coef_all.copy()
        flags["se_flag"] = np.where(
            flags["se"].isna(),
            "se_missing",
            np.where(flags["se"] <= 1e-12, "se_zero", np.where(flags["se"] > 1e3, "se_huge", "ok")),
        )
        flags["p_flag"] = np.where(flags["p_value"].isna(), "p_missing", "ok")
        flagged = flags[(flags["se_flag"] != "ok") | (flags["p_flag"] != "ok")].copy()
        flagged = flagged[["transition", "metric", "day_type", "event_day", "beta", "se", "p_value", "n_rows", "n_people", "n_dates", "se_flag", "p_flag"]]
        flagged.to_csv(table_dir / "06_estimation_flags.csv", index=False)

        eff = coef_all.copy()
        post = eff[(eff["event_day"] >= 0) & (eff["event_day"] <= 3)].copy()
        if not post.empty:
            summary = (
                post.groupby(["transition", "metric", "metric_label", "units", "day_type"], as_index=False)
                .agg(
                    post_window_beta_mean=("beta", "mean"),
                    post_window_beta_abs_max=("beta", lambda x: float(np.max(np.abs(x)))),
                    post_window_min_p=("p_value", "min"),
                    mean_se=("se", "mean"),
                    n_coef=("event_day", "size"),
                    n_rows=("n_rows", "max"),
                    n_people=("n_people", "max"),
                )
                .sort_values(["transition", "metric", "day_type"])
            )
            summary.to_csv(table_dir / "07_effect_summary_post0_to_3.csv", index=False)
    ...
    if not coef_all.empty:
        ...
        for metric_name, meta in metric_meta.items():
            plot_metric_transition_side_by_side(
                coef_all=coef_all,
                metric_name=metric_name,
                metric_label=meta["label"],
                units=meta["units"],
