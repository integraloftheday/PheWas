...
# %%
precomp_dir_candidates <- unique(c(
  Sys.getenv("PRECOMP_OUTPUT_DIR_05_6", ""),
  Sys.getenv("OUTPUT_DIR_05_5", ""),
  Sys.getenv("OUTPUT_DIR_05_7", ""),
  "results_05_5",
  "results_05_7"
))
precomp_dir_candidates <- precomp_dir_candidates[nzchar(precomp_dir_candidates)]

choose_precomp_dir <- function(candidates) {
  for (d in candidates) {
    tdir <- file.path(d, "tables")
    if (!dir.exists(tdir)) next
    if (file.exists(file.path(tdir, "predictions_all_05_5.csv")) ||
        file.exists(file.path(tdir, "predictions_all_05_7.csv"))) {
      return(d)
    }
  }
  if (length(candidates) > 0) candidates[[1]] else "results_05_5"
}

PRECOMP_OUTPUT_DIR <- choose_precomp_dir(precomp_dir_candidates)
PRECOMP_TABLE_DIR <- Sys.getenv("PRECOMP_TABLE_DIR_05_6", file.path(PRECOMP_OUTPUT_DIR, "tables"))
...
# %%
predictions_all <- safe_read_csv(c("predictions_all_05_5.csv", "predictions_all_05_7.csv"), required = TRUE)
model_inventory <- safe_read_csv(c("model_inventory_05_5.csv", "model_inventory_05_7.csv"), required = FALSE)

derived_main <- safe_read_csv(c("derived_duration_midpoint_main_grid_05_5.csv", "derived_duration_midpoint_main_grid_05_7.csv"), required = FALSE)
derived_main_compare <- safe_read_csv(c("derived_vs_direct_main_grid_05_5.csv", "derived_vs_direct_main_grid_05_7.csv"), required = FALSE)
derived_age <- safe_read_csv(c("derived_duration_midpoint_age_05_5.csv", "derived_duration_midpoint_age_05_7.csv"), required = FALSE)
derived_age_compare <- safe_read_csv(c("derived_vs_direct_age_05_5.csv", "derived_vs_direct_age_05_7.csv"), required = FALSE)

weekend_contrasts <- safe_read_csv(c("weekend_contrasts_05_5.csv", "weekend_contrasts_05_7.csv"), required = FALSE)
dst_contrasts <- safe_read_csv(c("dst_contrasts_05_5.csv", "dst_contrasts_05_7.csv"), required = FALSE)

write_table(predictions_all, "predictions_all_05_6_input_copy.csv")
if (nrow(model_inventory) > 0) write_table(model_inventory, "model_inventory_05_6.csv")

build_plot_origin_label <- function(pred_df, inventory_df, precomp_dir) {
  src <- attr(pred_df, "source_path") %||% file.path(precomp_dir, "tables", "predictions_all_05_5.csv")
...
if (require_cols(
  ...
  employment_weekend <- predictions_all %>%
  ...
  write_table(employment_weekend, "employment_x_weekend_plot_data_05_6.csv")
  ...
  if (nrow(timing_df) > 0) {
    ...
    save_plot(p_timing, "employment_weekend_timing_05_6.png", width = 15, height = 8)
  }
  ...
  if (nrow(duration_df) > 0) {
    ...
    save_plot(p_duration, "employment_weekend_duration_05_6.png", width = 11, height = 8)
  }
}
...
sex_main <- tibble()
if (require_cols(predictions_all, c("analysis", "sex_concept", "outcome", "batch", "estimate", "conf.low", "conf.high"), "predictions_all")) {
  sex_main <- predictions_all %>%
...
if (require_cols(predictions_all, c("analysis", "employment_status", "outcome", "batch", "estimate", "conf.low", "conf.high"), "predictions_all")) {
  employment_main <- predictions_all %>%
    filter(analysis == "employment_x_weekend") %>%
...
weekend_main <- tibble()
if (require_cols(predictions_all, c("analysis", "is_weekend_factor", "outcome", "batch", "estimate", "conf.low", "conf.high"), "predictions_all")) {
  weekend_main <- predictions_all %>%
...
if (nrow(main_effects) > 0) {
  write_table(main_effects, "main_effects_05_6.csv")
  ...
  if (nrow(main_timing) > 0) {
    ...
    save_plot(p_main_timing, "main_effects_timing_05_6.png", width = 15, height = 9)
  }
  ...
  if (nrow(main_duration) > 0) {
    ...
    save_plot(p_main_duration, "main_effects_duration_05_6.png", width = 12, height = 10)
  }
}
...
# %%
month_main <- predictions_all %>% filter(analysis == "month_main")
if (nrow(month_main) > 0 && require_cols(month_main, c("month", "outcome", "batch", "estimate", "conf.low", "conf.high"), "month_main")) {
  ...
  save_plot(p_month, "seasonality_month_main_05_6.png", width = 13, height = 9)
}

month_weekend <- predictions_all %>% filter(analysis == "month_x_weekend")
if (nrow(month_weekend) > 0 && require_cols(month_weekend, c("month", "is_weekend_factor", "outcome", "batch", "estimate"), "month_x_weekend")) {
  ...
  write_table(month_weekend, "month_x_weekend_05_6.csv")
  ...
  save_plot(p_month_weekend, "seasonality_month_x_weekend_05_6.png", width = 15, height = 8)
}

month_dst <- predictions_all %>% filter(analysis == "month_x_dst", outcome %in% c("onset", "offset"))
if (nrow(month_dst) > 0 && require_cols(month_dst, c("month", "dst_observes", "outcome", "batch", "estimate", "conf.low", "conf.high"), "month_x_dst")) {
  ...
  write_table(month_dst, "onset_offset_month_x_dst_05_6.csv")
  ...
  save_plot(p_month_dst, "onset_offset_month_x_dst_05_6.png", width = 13, height = 8)
}
...
# %%
age_main <- predictions_all %>% filter(analysis == "age_main")
if (nrow(age_main) > 0 && require_cols(age_main, c("age_at_sleep", "outcome", "batch", "estimate", "conf.low", "conf.high"), "age_main")) {
  ...
  write_table(age_main, "age_main_05_6.csv")
  ...
  if (nrow(age_timing) > 0) {
    ...
    save_plot(p_age_timing, "age_trends_timing_05_6.png", width = 15, height = 6)
  }
  ...
  if (nrow(age_duration) > 0) {
    ...
    save_plot(p_age_duration, "age_trends_duration_05_6.png", width = 11, height = 8)
  }
}

age_x_employment_duration <- predictions_all %>% filter(analysis == "age_x_employment", outcome == "duration")
if (nrow(age_x_employment_duration) > 0 && require_cols(age_x_employment_duration, c("age_at_sleep", "employment_status", "batch", "estimate", "conf.low", "conf.high"), "age_x_employment_duration")) {
  ...
  write_table(age_x_employment_duration, "duration_age_x_employment_05_6.csv")
  ...
  save_plot(p_age_emp, "duration_age_x_employment_05_6.png", width = 12, height = 9)
}

age_x_dst_onoff <- predictions_all %>% filter(analysis == "age_x_dst", outcome %in% c("onset", "offset"))
if (nrow(age_x_dst_onoff) > 0 && require_cols(age_x_dst_onoff, c("age_at_sleep", "dst_observes", "outcome", "batch", "estimate", "conf.low", "conf.high"), "age_x_dst")) {
  ...
  write_table(age_x_dst_onoff, "onset_offset_age_x_dst_05_6.csv")
  ...
  save_plot(p_age_dst, "onset_offset_age_x_dst_05_6.png", width = 13, height = 8)
}
...
# %%
if (nrow(derived_age_compare) > 0 && require_cols(derived_age_compare, c("age_at_sleep", "batch", "derived_duration_hours", "duration_direct_estimate"), "derived_age_compare")) {
  derived_age_duration <- derived_age_compare %>%
    select(any_of(c("batch", "age_at_sleep", "is_weekend_factor", "dst_observes", "derived_duration_hours", "duration_direct_estimate"))) %>%
    pivot_longer(
  ...
  write_table(derived_age_duration, "derived_vs_direct_duration_age_plot_data_05_6.csv")

  p_derived_age <- ggplot(
    ...
    scale_y_continuous(labels = format_duration_axis) +
    labs(
      title = "Reconstructing Duration from Onset + Offset",
      subtitle = "Age-grid comparison of derived vs direct duration",
      x = "Age (years)",
      y = "Predicted Sleep Duration",
      color = NULL
    ) +
  ...
  save_plot(p_derived_age, "derived_vs_direct_duration_age_05_6.png", width = 16, height = 8)
}

if (nrow(derived_main_compare) > 0 && require_cols(derived_main_compare, c("batch", "derived_duration_hours"), "derived_main_compare")) {
  derived_main_compare <- derived_main_compare %>%
    filter(!is.na(duration_direct_minus_derived)) %>%
  ...
  if ("duration_direct_minus_derived" %in% names(derived_main_compare)) {
    p_main_diff <- ggplot(derived_main_compare, aes(x = duration_direct_minus_derived, y = reorder(strata, duration_direct_minus_derived), color = batch)) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "gray60") +
      geom_point(size = 2.3) +
      labs(
        title = "Direct minus Derived Duration (Main Grid)",
        x = "Hours (Direct - Derived)",
        y = NULL,
        color = "Batch"
      ) +
      theme_classic(base_size = 12)

    save_plot(p_main_diff, "derived_vs_direct_duration_main_grid_05_6.png", width = 12, height = 8)
  }

  write_table(derived_main_compare, "derived_vs_direct_main_grid_05_6.csv")
}

if (nrow(derived_main) > 0) write_table(derived_main, "derived_midpoint_duration_main_grid_05_6.csv")
if (nrow(derived_age) > 0) write_table(derived_age, "derived_midpoint_duration_age_05_6.csv")
if (nrow(derived_age_compare) > 0) write_table(derived_age_compare, "derived_vs_direct_age_05_6.csv")
...
if (nrow(weekend_contrasts) > 0 && require_cols(weekend_contrasts, c("outcome", "contrast_scope", "weekend_minus_weekday"), "weekend_contrasts")) {
  ...
  write_table(weekend_contrasts, "weekend_contrasts_05_6.csv")
  ...
  save_plot(p_weekend_ctr, "weekend_contrasts_05_6.png", width = 16, height = 10)
}
...
if (nrow(dst_contrasts) > 0 && require_cols(dst_contrasts, c("outcome", "contrast_scope", "nodst_minus_dst"), "dst_contrasts")) {
  ...
  write_table(dst_contrasts, "dst_contrasts_05_6.csv")
  ...
  save_plot(p_dst_ctr, "dst_contrasts_05_6.png", width = 16, height = 10)
}
...
# %%
writeLines(analysis_log, con = file.path(OUTPUT_DIR, "analysis_log_05_6.txt"))
log_msg("Done.")
